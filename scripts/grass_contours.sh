...
try_and_log g.region -a `echo $region_string | sed -e 's/res=.*/res=100/g'`
...
try_and_log v.out.ogr -s -e input=${vec_basename}_simplified20 type=line output=$tmp_folder/${vec_basename}_simplified20.shp format=ESRI_Shapefile output_layer=output --overwrite" > ${tmp_folder}/my_grassjob.sh

# 
# verify the content of the file
cat ${tmp_folder}/my_grassjob.sh
...
xmin=`python -c "print max($west ,0)"`
ymin=`python -c "print max($south ,0)"`
xmax=`python -c "print max($east , $xmin)"`
ymax=`python -c "print max($north ,$ymin)"`

echo "xmin: $xmin, ymin: $ymin, xmax: $xmax, ymax: $ymax, buffer_src_coords: $buffer_src_coords"
# Cut shape file
